...
int tag_help;//size of tag in bits, help calculate tag for comparison
int share;//what kind of share is the SMT_entry calculated by
...
int BP_init(unsigned btbSize, unsigned historySize, unsigned tagSize, unsigned fsmState,
    ...
    sim.size = (tagSize + 30 + VALID_BITS) * btbSize;
    for (i = 0; i < btbSize; i++) {
        BTB[i].PC = 0;
        BTB[i].tag = 0;
    }
...
bool BP_predict(uint32_t pc, uint32_t *dst){
    ...
    uint32_t tag = calc_tag(pc);//calculate tag for comparison;
...
void BP_update(uint32_t pc, uint32_t targetPc, bool taken, uint32_t pred_dst){
    ...
    uint32_t tag = calc_tag(pc);//tag for comparison;
    uint8_t state = 0;
